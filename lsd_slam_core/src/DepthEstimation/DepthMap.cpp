...
void DepthMap::observeDepthRow(int yMin, int yMax, RunningStats* stats)
{
	...
	for(int y=yMin;y<yMax; y++)
		for(int x=3;x<width-3;x++)
		{
            ...
            //新点:构造深度；旧点:更新深度
			if(!hasHypothesis)
                //如果当前关键帧的该像素没有被深度估计，就尝试创建
				success = observeDepthCreate(x, y, idx, stats);
			else
                //如果当前关键帧的该像素已经被深度估计，那么就更新它
...
///
/// \brief DepthMap::makeAndCheckEPL
/// \param x
/// \param y
/// \param ref
/// \param pepx
...
//判断参考帧是用旧的还是新的
	...
	float epx, epy;
    //在非关键帧上计算极线方向，去除极线和梯度垂直或者极线上梯度不分明的情况
...
///
/// \brief DepthMap::propagateDepth
/// \param new_keyframe
///创建新的关键帧之后，为当前帧创建深度地图，更新currentDepthMap，完成深度地图创建
/// 方式:将参考关键帧的点直接投影到当前参考帧，若光度误差较小，则创建深度
void DepthMap::propagateDepth(Frame* new_keyframe)
{
    ...
    const float* activeKFImageData = activeKeyFrame->image(0);//参考帧(当前关键帧)
	const float* newKFMaxGrad = new_keyframe->maxGradients(0);
    const float* newKFImageData = new_keyframe->image(0);//当前帧


//std::cout<<new_keyframe->pose->frameID<<' '<<activeKeyFrame->id()<<std::endl;
	...
	for(int y=0;y<height;y++)
		for(int x=0;x<width;x++)
		{
			...
			{
                //std::cout<<"already have pixle"<<std::endl;
				if(enablePrintDebugInfo) runningStats.num_prop_merged++;
                //之前直接传导得到的深度和已经存在的深度进行卡尔曼融合
...
void DepthMap::regularizeDepthMapFillHolesRow(int yMin, int yMax, RunningStats* stats)
{
	...
	for(int y=yMin; y<yMax; y++)
	{
		for(int x=3;x<width-2;x++)
		{
            ...
            //todo:otherDepthMap在之前已经被currentDepthMap所替换
			DepthMapPixelHypothesis* dest = otherDepthMap + idx;
            if(dest->isValid) continue;//只更新还没有深度的点
            if(keyFrameMaxGradBuf[idx]<MIN_ABS_GRAD_DECREASE) continue;//梯度要大于阈值

			int* io = validityIntegralBuffer + idx;
            //记录需要地图填充的个数
            //todo:在当前像素附近一个6x6的格子范围内
			int val = io[2+2*width] - io[2-3*width] - io[-3+2*width] + io[-3-3*width];

            //判断有效点个数是否满足要求(在5x5格子内至少30个有效观测次数)
...
///regularizeDepthMapRow作用:实现深度地图的融合，平滑，并去除部分不好的点
///validityTH:在5x5格子中的像素有效观测次数：25
///
///
...
///
/// \brief DepthMap::updateKeyframe
/// \param referenceFrames
///作用:Depth Map Refinement
...
///
/// \brief DepthMap::createKeyFrame
/// \param new_keyframe
///创建新的关键帧，构建它的深度地图，并把深度地图的平均深度和关键帧的位姿归一化，方便后续的sim3优化
void DepthMap::createKeyFrame(Frame* new_keyframe)
{
	...
	struct timeval tv_start, tv_end;
	gettimeofday(&tv_start, NULL);
    //为当前帧创建深度地图，即刷新currentDepthMap(把currentDepthMap用new_keyframe刷新)
    ...
    //将当前帧的像素点的平均深度归一化，对应的thisToParent_raw也会被归一化
    //主要是为后面的sim3优化提供方便
...
void DepthMap::finalizeKeyFrame()
{
	...
	gettimeofday(&tv_start, NULL);
    //在已知部分像素深度信息的前提下，通过周围像素的深度融合，实现部分未知深度的像素的深度估计
	regularizeDepthMapFillHoles();
	gettimeofday(&tv_end, NULL);
	msFillHoles = 0.9*msFillHoles + 0.1*((tv_end.tv_sec-tv_start.tv_sec)*1000.0f + (tv_end.tv_usec-tv_start.tv_usec)/1000.0f);
	nFillHoles++;
	...
	RunningStats* stats)
{
    ...
    Eigen::Vector3f KinvP = Eigen::Vector3f(fxi*u+cxi,fyi*v+cyi,1.0f);//在当前关键帧下的归一化坐标
	Eigen::Vector3f pInf = referenceFrame->K_otherToThis_R * KinvP;
    Eigen::Vector3f pReal = pInf / prior_idepth + referenceFrame->K_otherToThis_t;//在参考帧下的归一化坐标
//深度的比值???(应该是深度的误差量)
...
//计算远点的数据
	...
	while(((incx < 0) == (cpx > pClose[0]) && (incy < 0) == (cpy > pClose[1])) || loopCounter == 0)
	{
        ...
        //只需要计算做前面的一个点，其他的点都是从之前的计算复制过来的
	...
	else
	{
        ...
        idnew_best_match = (dot1 - oldY*dot2) / nominator;//深度值
