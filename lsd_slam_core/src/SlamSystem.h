...
private:
    ...
    TrackingReference* trackingReference; //利用keyframe的点云 tracking reference for current keyframe. only used by tracking.
	SE3Tracker* tracker;
    ...
    std::deque< std::shared_ptr<Frame> > unmappedTrackedFrames;//还没有创建的地图，用来refine关键帧
    ...
    std::shared_ptr<Frame> trackingReferenceFrameSharedPT;	// slam中没有什么用only used in odometry-mode, to keep a keyframe alive until it is deleted. ONLY accessed whithin currentKeyFrameMutex lock.
