...
///
/// \brief SlamSystem::mergeOptimizationOffset
///作用:将当前地图中所有的关键帧都更新成用g2o优化后的位姿
...
void SlamSystem::finishCurrentKeyframe()
{
    ...
    //在finalizeKeyFrame函数中，主要实现在当前关键帧中
    //通过深度融合添加在深度点周围的深度点，并做了深度地图的平滑，去除了部分坏点
...
///
/// \brief SlamSystem::changeKeyframe
/// \param noCreate
/// \param force
/// \param maxScore
///先从关键帧序列中找可以替代当前帧的关键帧，若有就用该关键帧替代当前帧作为当前关键帧(省去了创建的步骤)
/// 若没有找到合适的就只能创建关键帧了
...
//用当前tracking到的非关键帧去更新关键帧，增加和优化它的深度点
...
///
/// \brief SlamSystem::doMappingIteration
/// \return
///主要作用:
/// 1、将当前地图中所有的关键帧都更新成用g2o优化后的位姿
/// 2、将tracking得到的还没被深度估计的帧做深度估计
/// 3、更新了currentKeyFrame用作tracking
bool SlamSystem::doMappingIteration()
{
	if(currentKeyFrame == 0)
		return false;
    //若不建图，判断当前关键帧是否加入到了地图做了融合处理(这里不会用到)
	if(!doMapping && currentKeyFrame->idxInKeyframes < 0)
	{
        //若当前关键帧被观测到的频率很高，就把关键帧加入地图，做融合处理
	...
	if(trackingIsGood)
	{
        ...
        //todo:createNewKeyFrame为true说明需要为tracking寻找一个更加合适的关键帧
		...
		{
            //若不需要添加关键帧，则updateKeyframe，将unmappedTrackedFrames融合到当前关键帧
...
//跟踪线程
///
/// \brief SlamSystem::trackFrame
/// \param image
/// \param frameID
/// \param blockUntilMapped
/// \param timestamp
///当前帧和参考关键帧的传递
/// 首先给定当前帧的位姿为上一帧的位姿，将trackingReference的点云投影到当前帧，并用带权重的光度误差函数作为目标，使用LM算法完成位姿优化
/// 判断是否添加关键帧或者refine当前关键帧
void SlamSystem::trackFrame(uchar* image, unsigned int frameID, bool blockUntilMapped, double timestamp)
{
    ...
    //将前一个跟踪成功的帧与参考帧的变换作为初始估计（其实也就假设了前一帧的位姿就是当前帧的位姿)
    //TODO:frameToReference指的是Trf,也就是将Reference帧的点变换到frame帧的点所需要的变换
    ...
    //把每一个跟踪成功的帧都插入到keyFrameGraph(这里作为一个普通的帧，所以仅仅加入了pose信息到地图)
    //目的是为了给下一帧提供位姿参考
	...
	// Keyframe selection
    //latestTrackedFrame是可能作为新关键帧的
	latestTrackedFrame = trackingNewFrame;
    //如果之前没有新的关键帧，且当前帧满足关键帧的要求
...
///
/// \brief SlamSystem::tryTrackSim3
/// \param A新关键帧
/// \param B候选关键帧
/// \param lvlStart金字塔开始层数4
/// \param lvlEnd金字塔结束层数3
/// \param useSSE
/// \param AtoB
/// \param BtoA
/// \param e1返回sim3结果
...
///
/// \brief SlamSystem::testConstraint
/// \param candidate
/// \param e1_out返回候选帧与新关键帧的sim3结果
/// \param e2_out返回候选帧与新关键帧的sim3结果
/// \param candidateToFrame_initialEstimate
/// \param strictness
///
