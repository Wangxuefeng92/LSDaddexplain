 ...
 */
//用已知的keyframe的3D点云的投影来实现新帧的位姿跟踪
...
public:
	...
	void clearAll();
	void invalidate();
    Eigen::Vector3f* posData[PYRAMID_LEVELS];	// (x,y,z)存放那个这参考点的3D坐标(在当前参考系下)
	Eigen::Vector2f* gradData[PYRAMID_LEVELS];	// (dx, dy)
	Eigen::Vector2f* colorAndVarData[PYRAMID_LEVELS];	// (I, Var)
    int* pointPosInXYGrid[PYRAMID_LEVELS];	// x + y*width表示存放在图像中的位置(坐标)
    int numData[PYRAMID_LEVELS];//记录多少点被3角化
