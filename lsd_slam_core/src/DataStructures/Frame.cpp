...
Frame::Frame(int id, int width, int height, const Eigen::Matrix3f& K, double timestamp, const unsigned char* image)
{
    ...
    //拷贝图像，从uchar变成了float
    data.image[0] = FrameMemory::getInstance().getFloatBuffer(data.width[0]*data.height[0]);//申请内存
...
//如果对应的金字塔已经存在，则直接返回，不用创建
//构建图像金字塔，方法是4个像素平均变成一个像素
...
///
/// \brief Frame::buildGradients
/// \param level
///记录下当前level下，图像的a.水平梯度；b.竖直梯度；c.实际像素值
void Frame::buildGradients(int level)
{
	...
	{
		val_p1 = *(img_pt+1);
        //当前像素右边和左边两个像素的之差除以2,得到当前像素的水平方向的梯度
        *((float*)gradxyii_pt) = 0.5f*(val_p1 - val_m1);
        //当前像素下边和上边两个像素的之差除以2,得到当前像素的竖直方向的梯度
